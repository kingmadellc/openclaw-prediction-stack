...
def _get_client(*, _retries: int = 1, _backoff: float = 2.0):
    ...
    for attempt in range(_retries + 1):
        try:
            ...
            # Verify auth — use raw API call to avoid SDK Pydantic deserialization issues
            try:
                _verify_url = f"{BASE_URL}/portfolio/positions?limit=1"
                _verify_resp = client.call_api("GET", _verify_url)
                _verify_data = json.loads(_verify_resp.read())
                if isinstance(_verify_data, dict) and "error" in _verify_data:
                    raise RuntimeError(_verify_data["error"])
            except (TypeError, AttributeError):
                # Fallback for older SDK versions without call_api
...
def _place_order(
    ...
    try:
        ...
        # Use raw HTTP — SDK Pydantic models break across versions
        order_url = f"{BASE_URL}/portfolio/orders"
        resp = client.call_api("POST", order_url, body=order_params)
...
def execute_pick_command(pick_number: int, qty_override: int = 0) -> str:
    ...
    if status not in ("open", "active"):
        return f"❌ Market {ticker} is not tradable (status: {status}). Cannot trade."
