...
def _get_active_kalshi_topics() -> list:
    ...
    try:
        ...
        with open(private_key_file) as f:
            private_key = f.read().strip()  # Strip trailing whitespace — prevents RSA signature corruption

        config = Configuration(
            host="https://api.elections.kalshi.com/trade-api/v2"
        )
        config.private_key_pem = private_key  # PEM MUST be set before api_key_id
...
if __name__ == "__main__":
    ...
    if args.test_slack:
        _notify_slack("XPULSE TEST: Slack notification is working.")
