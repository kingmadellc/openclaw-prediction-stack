...
def _position_qty(position):
    """Return signed integer position size across known Kalshi schemas."""
    try:
        return int(float(position.get("position_fp") or position.get("position") or position.get("quantity") or 0))
    except (TypeError, ValueError):
        return 0


def _position_average_price_cents(position):
    """Return average entry price in cents from raw position data."""
    for key in ("average_price", "avg_price", "price"):
        ...
        try:
            fval = float(value)
            return int(round(fval * 100)) if fval <= 1.01 else int(round(fval))
        except (TypeError, ValueError):
            continue
    return 0


def _fetch_positions_raw(kalshi):
    """Fetch positions from raw API payloads to avoid typed SDK deserialization."""
    resp = kalshi._portfolio_api.get_positions_without_preload_content(limit=100)
    data = json.loads(resp.read())
    return data.get("event_positions") or data.get("positions") or data.get("market_positions") or []


def _fetch_market_raw(kalshi, ticker):
    """Fetch a single market via raw API and normalize v3 dollar/fp fields."""
    url = f"{KALSHI_BASE_URL}/markets/{ticker}"
    data = json.loads(kalshi.call_api("GET", url).read())
...
def format_activity_section(kalshi, config, debug=False):
    ...
    try:
        positions = [p for p in _fetch_positions_raw(kalshi) if _position_qty(p) != 0]
        total_cost = 0.0
        total_unrealized = 0.0

        for position in positions:
            qty = abs(_position_qty(position))
            avg_price_cents = _position_average_price_cents(position)
            total_cost += qty * avg_price_cents / 100.0

            current_price_cents = avg_price_cents
            ticker = position.get("ticker")
            if ticker:
                try:
                    market = _fetch_market_raw(kalshi, ticker)
                    current_price_cents = market.get("last_price") or market.get("yes_ask") or avg_price_cents
                except Exception:
                    pass

            total_unrealized += qty * (current_price_cents - avg_price_cents) / 100.0
