...
def load_config(config_path=None):
    ...
    Supports two config layouts:
      1. Dedicated brief config: data["market_morning_brief"] section
      2. Top-level OpenClaw config: data["kalshi"] at root (e.g. ~/.openclaw/config.yaml)
    Falls back to ~/.openclaw/config.yaml automatically if no --config given.
    """
    defaults = {
        "enabled": True,
    ...
    # Resolve config path — fall back to ~/.openclaw/config.yaml
    paths_to_try = []
    if config_path:
        paths_to_try.append(Path(config_path))
    paths_to_try.append(Path.home() / ".openclaw" / "config.yaml")
    ...
    for p in paths_to_try:
        if p.exists() and yaml:
            ...
            # Layout 1: dedicated market_morning_brief section
            if "market_morning_brief" in data:
                return data["market_morning_brief"]

            # Layout 2: top-level OpenClaw config — merge kalshi into defaults
            if "kalshi" in data:
                merged = dict(defaults)
                merged["kalshi"] = data["kalshi"]
                return merged
...
def main():
    ...
    parser.add_argument("--evening", action="store_true", help="Generate evening brief variant")
    ...
    # --evening delegates to evening_brief.py (separate script)
    if args.evening:
        import subprocess as _sp
        _evening_script = Path(__file__).parent / "evening_brief.py"
        _cmd = [sys.executable, str(_evening_script)]
        if args.config:
            _cmd.extend(["--config", args.config])
        if args.debug:
            _cmd.append("--debug")
        if args.dry_run:
            _cmd.append("--dry-run")
        result = _sp.run(_cmd, capture_output=False)
