...
def _apply_ensemble_weights(edges: list, weights: dict) -> list:
    """Blend Kalshalyst edges with Xpulse social signals using ensemble weights.

    If Xpulse has a corroborating signal for a market (matching ticker in cache),
    the edge confidence gets boosted by the Xpulse weight. If no signal, only
    Kalshalyst weight applies. Uses confidence_weighted combination method.

    Args:
        edges: List of edge dicts from calculate_edges()
        weights: Dict with kalshalyst/xpulse weights and combination_method
    Returns:
        edges list with adjusted confidence scores
    """
    w_k = weights.get("kalshalyst", 0.76)
    w_x = weights.get("xpulse", 0.24)
    min_sources = weights.get("min_sources_required", 1)

    # Load Xpulse signal cache
    xpulse_signals = {}
    state_dir = Path.home() / ".openclaw" / "state"
    cache_paths = [
        state_dir / "x_signal_cache.json",
        Path.home() / ".x_signal_cache.json",
    ]
    for cp in cache_paths:
        if cp.exists():
            try:
                with open(cp) as f:
                    cache = json.load(f)
                # Index signals by any ticker mentions
                for sig in cache.get("signals", []):
                    topic = (sig.get("topic") or "").lower()
                    ticker = (sig.get("ticker") or "").upper()
                    confidence = sig.get("confidence", 0.5)
                    if ticker:
                        xpulse_signals[ticker] = confidence
                    # Also index by topic keywords for fuzzy matching
                    for word in topic.split():
                        if len(word) > 3:
                            xpulse_signals[word.upper()] = confidence
                logger.info(f"Ensemble: loaded {len(cache.get('signals', []))} Xpulse signals")
                break
            except Exception as e:
                logger.warning(f"Ensemble: failed to load Xpulse cache from {cp}: {e}")

    boosted = 0
    for edge in edges:
        ticker = edge.get("ticker", "").upper()
        title_words = [w.upper() for w in (edge.get("title") or "").split() if len(w) > 3]

        # Check for corroborating Xpulse signal
        xpulse_conf = 0.0
        if ticker in xpulse_signals:
            xpulse_conf = xpulse_signals[ticker]
        else:
            # Fuzzy: check if any title word matches a signal topic
            for w in title_words:
                if w in xpulse_signals:
                    xpulse_conf = max(xpulse_conf, xpulse_signals[w])

        # Apply ensemble weighting
        k_conf = edge.get("confidence", 0.5)
        if xpulse_conf > 0:
            # Confidence-weighted blend: both sources corroborate
            blended = (w_k * k_conf + w_x * xpulse_conf) / (w_k + w_x)
            edge["confidence"] = round(blended, 4)
            edge["ensemble_sources"] = 2
            edge["xpulse_conf"] = round(xpulse_conf, 4)
            boosted += 1
        else:
            # Single source: apply Kalshalyst weight only (slight discount)
            edge["confidence"] = round(k_conf * w_k, 4)
            edge["ensemble_sources"] = 1

    logger.info(f"Ensemble: {boosted}/{len(edges)} edges boosted by Xpulse signals")
    return edges
...
def run_auto_trader(dry_run: bool = False) -> dict:
    ...
    try:
        ...
        # Load ensemble weights if config exists
        ensemble_weights = None
        ensemble_weights_path = Path.home() / "prompt-lab" / "ensemble_weights.json"
        if ensemble_weights_path.exists():
            try:
                with open(ensemble_weights_path) as f:
                    ensemble_weights = json.load(f)
                pipeline_cfg["ensemble_enabled"] = True
                logger.info(
                    f"Ensemble weights loaded: kalshalyst={ensemble_weights.get('kalshalyst', 0):.2f}, "
                    f"xpulse={ensemble_weights.get('xpulse', 0):.2f}, "
                    f"method={ensemble_weights.get('combination_method', 'unknown')}"
                )
            except Exception as e:
        ...
        # ── Phase 4.6: Ensemble weighting (Xpulse signal boost) ──────
        if ensemble_weights and pipeline_cfg.get("ensemble_enabled"):
            edges = _apply_ensemble_weights(edges, ensemble_weights)
