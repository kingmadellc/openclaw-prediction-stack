...
def _coerce_cents(val, default=0):
    """Convert either dollar floats/strings or integer cents to cents."""
    if val is None:
        return default
    try:
        fval = float(val)
...
def _fetch_market_raw(client, ticker: str) -> dict:
    """Fetch a single market via raw API and normalize the nested market payload."""
...
def _extract_market_outcome(market: dict) -> Optional[int]:
    """Infer resolved YES/NO outcome from explicit market resolution fields."""
    if str(market.get("status", "")).lower() != "resolved":
        return None
    ...
    for value in candidates:
        ...
        if normalized in ("yes", "1", "100", "true"):
            return 1
        if normalized in ("no", "0", "false"):
            return 0

    yes_settlement = market.get("yes_settlement")
    if yes_settlement is not None:
        settlement_cents = _coerce_cents(yes_settlement, default=-1)
        if settlement_cents in (0, 100):
            return 1 if settlement_cents == 100 else 0

    no_settlement = market.get("no_settlement")
    if no_settlement is not None:
        settlement_cents = _coerce_cents(no_settlement, default=-1)
        if settlement_cents in (0, 100):
            return 0 if settlement_cents == 100 else 1

    return None


def _position_market_price_cents(position: dict) -> int:
    """Best-effort current position price in cents across v2/v3 position payloads."""
    for key in ("last_traded_price", "last_price", "yes_price"):
        if position.get(key) is not None:
            return _coerce_cents(position.get(key), default=50)
    for key in ("last_traded_price_dollars", "last_price_dollars", "yes_price_dollars"):
        if position.get(key) is not None:
            return _coerce_cents(position.get(key), default=50)
    return 50
...
def auto_resolve_from_kalshi(client, log_path: Optional[Path] = None) -> list[Trade]:
    ...
    for trade in open_trades:
        try:
            market_data = _fetch_market_raw(client, trade.ticker)

            outcome = _extract_market_outcome(market_data)
            if outcome is not None:
                resolved = logger_obj.resolve_trade(trade.ticker, outcome)
                resolved_trades.append(resolved)

            elif str(market_data.get("status", "")).lower() == "resolved":
                logger.warning(f"Resolved market {trade.ticker} has no explicit outcome field; skipping auto-resolve")
    ...
    try:
        ...
        try:
            ...
            for p in all_positions:
                ...
                market_price_cents = _position_market_price_cents(p)
