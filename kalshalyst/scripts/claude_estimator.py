...
def _claude_cli_estimate(prompt: str, system: str, timeout: int = 60) -> Optional[dict]:
    ...
    try:
        ...
        # Build explicit env with ANTHROPIC_API_KEY guaranteed present
        cli_env = os.environ.copy()
        if "ANTHROPIC_API_KEY" not in cli_env:
            key = _load_anthropic_key()
            if key:
                cli_env["ANTHROPIC_API_KEY"] = key
        ...
        result = subprocess.run(
            ...
            env=cli_env,  # Explicit env — start_new_session can break inheritance
        )
...
def _claude_api_estimate(prompt: str, system: str, timeout: int = 45) -> Optional[dict]:
    ...
    try:
        ...
        client = anthropic.Anthropic(api_key=api_key)  # Explicit key — never rely on implicit env
...
def _load_system_prompt() -> str:
    """Load system prompt — local config from ~/prompt-lab/prompt.md, else built-in defaults."""
