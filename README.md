...
Eight autonomous skills that scan, analyze, compare, and trade prediction markets — with a setup wizard that wires them into a proactive, scheduled system delivering intelligence to your phone.

<!-- CODEX: synchronized public stack docs with the reference implementation and added an explicit verification path. -->
**v3.0** — 8-skill stack: 7 runtime skills plus the setup wizard. All published on [ClawHub](https://clawhub.ai).
...
| 8 | [**Prediction Stack Setup**](prediction-stack-setup/) | Setup wizard — detects installed skills, walks through API keys, creates cron jobs, enables heartbeat, tests iMessage delivery. Wires the stack into one operating system. |
...
│    AUTO-TRADER LAYER    │   │  Polymarket CC → market data  │
│  auto_trader.py routes  │   └───────────────────────────────┘
│  edges → Kelly sizing   │
│  → execution via CC     │
└─────────────────────────┘
```
